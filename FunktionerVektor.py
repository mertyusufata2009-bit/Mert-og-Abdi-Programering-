...
def Vektor2Denhedsvektor(x1, y1):
    længde = Vektor2Dlængde(x1, y1)
    tjek_nulvektor(længde)

    return x1 / længde, y1 / længde


def VinkelMellem2D(x1, y1, x2, y2):
    længde1 = Vektor2Dlængde(x1, y1)
    længde2 = Vektor2Dlængde(x2, y2)

    tjek_nulvektor(længde1)
    tjek_nulvektor(længde2)

    cos_vinkel = Vektor2Dprikprodukt(x1, y1, x2, y2) / (længde1 * længde2)
...
def Projektion2D(x1, y1, x2, y2):
    ...
    længde_b_i_anden = x2**2 + y2**2

    if længde_b_i_anden == 0:
        raise ValueError("Man kan ikke projicere på nulvektoren.")

    faktor = prik / længde_b_i_anden
...
def Vektor3Denhedsvektor(x1, y1, z1):
    længde = Vektor3Dlængde(x1, y1, z1)
    tjek_nulvektor(længde)

    return x1 / længde, y1 / længde, z1 / længde


def VinkelMellem3D(x1, y1, z1, x2, y2, z2):
    længde1 = Vektor3Dlængde(x1, y1, z1)
    længde2 = Vektor3Dlængde(x2, y2, z2)

    tjek_nulvektor(længde1)
    tjek_nulvektor(længde2)

    cos_vinkel = Vektor3Dprikprodukt(x1, y1, z1, x2, y2, z2) / (længde1 * længde2)
